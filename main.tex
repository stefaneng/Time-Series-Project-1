...
We can also see that $\Var(cos(wt + Y)) < +\infty$
\begin{align*}
\Var(cos(wt + Y)) &= \Cov(cos(wt + Y), cos(wt + Y))\\
&= E[cos^2(wt + Y)] - \underbrace{E[cos(wt + Y)]^2}_{0}\\
&= E[\frac{1 + \cos(2(wt + Y))}{2}] < \infty
\end{align*}

\begin{align*}
\gamma_X(t,t + h) &= Cov(X_{t}, X_{t + h}) = E[cos(wt + Y)cos(w(t+h) + Y)] \\
 &= E[(\cos(wt)\cos(Y) - \sin(wt)\sin(Y))(\cos(wt + wh))\cos(Y) - \sin(wt + wh))\sin(Y)] \\
 &= E[\cos^2(Y)\cos(wt)\cos(wt + wh) + \sin^2(Y)sin(wt)sin(wt + wh)\\
 & \quad + \cos(wt)\sin(wt + wh)\sin(Y)\cos(Y) - \sin(wt)\cos(wt + wh)\sin(Y)\cos(Y)]\\
 & = \underbrace{E[\cos^2(Y)]}_{\pi \text{ by Appendix \ref{appendix:cos2y}}} \cos(wt)\cos(wt + wh) + \underbrace{E[\sin^2(Y)]}_{\pi \text{ by Appendix \ref{appendix:cos2y}}} sin(wt)sin(wt + wh) +\\
 & \quad \underbrace{E[sin(Y)cos(Y)]}_{\text{0 by Appendix \ref{appendix:sinycosy}}} [\cos(wt)\sin(wt + wh) - \sin(wt)\cos(wt + wh)]\\
 &= \pi [\cos(wt)\cos(wt + wh) + sin(wt)sin(wt + wh)]\\
 &= \pi [
 \cos(wt)[\cos(wt) \cos(wh) - \sin(wt) \sin(wh)] +\\
 &\qquad sin(wt)[sin(wt) cos(wh) + sin(wh) cos(wt)]]\\
 &= \pi [
  cos^2(wt) cos(wh) + sin^2(wt) cos(wh) +\\
  &\qquad sin(wh)cos(wt)sin(wt) - sin(wh)cos(wt)sin(wt)
 ]\\
 &= \pi cos(wh) [cos^2(wt) + sin^2(wt)]\\
 &= \pi \cos(wh)
 \end{align*}

 Since $\gamma_X(t,t + h)$ does not depend on $h$, this completes the proof that the process to be stationary.

It follows that the autocovariance function for $X_t$ is $\gamma_X(h) = \pi \cos(wh)$.
